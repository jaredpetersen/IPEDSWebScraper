...
def get_data(page, page_header):
     ...
     if soup.find('table', 'ipdGlsSearch') is None:
          ...
          for tr in soup.find_all('tr')[1:]:
               tds = tr.find_all('td')
               # The encoding is technically ISO-8859-1, but that results in some mystery characters
               write_data(tds[0].text.encode('utf-8'), tds[1].text.encode('utf-8'))

          # Close the table
          write_list.append('</tbody>\n</table>\n<br />\n')

def write_data(term, definition):
